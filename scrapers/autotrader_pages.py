...
def getCarsFromAllPages(first_page_url, max_pages, usedCarsdf):
    ...
    for i in range(max_pages):
        ...
        while list_page.status_code != 200 and list_page.status_code != 404:
        ...
        for link in link_list:
            ...
            while basic_json.status_code != 200 and basic_json.status_code != 404:
                print(link)
            ...
            if car_json is not None:
                specs1 = car_json.get('pageData')
                if specs1 is not None:
                    specs = specs1.get('tracking')
                else:
                    specs = car_json
            else:
                specs = car_json
            ...
            while specification_json.status_code != 200 and specification_json.status_code != 404:
                print('Got response:', specification_json.status_code, 'going to sleep for 3min')
                sleep(60*3)
                specification_json = requests.get("https://www.autotrader.co.uk/json/fpa/lazy/"+link[link.find('/20')+1:link.find('?')], headers=headers)
            car_tech_specs = specification_json.json()

            if basic_json.status_code == 404 or specification_json.status_code == 404 or list_page.status_code == 404:
                print("Run on some 404 error on link:",link)
            else:
                area=""
                ...
                if specs.get('average_mpg') is not None and float(specs.get('average_mpg')) != 0:
                    consumption.append(235.214583/float(specs.get('average_mpg')))
                else:
                    consumption.append(specs.get('average_mpg'))

                if specs.get('mileage') is not None and specs.get('mileage') != 'null':
                    mileage.append(float(specs.get('mileage'))*1.609)
                else:
                    mileage.append(specs.get('mileage'))
